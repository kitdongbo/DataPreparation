...
def WriteDataSet(f_path, data_set):
    ...
    header = [u"№", "PtId", "DT_Fix", "NoctMin_T", "NoctMin_Gl"]
    ...
    for index, record in enumerate(data_set, start=1):
        ...
        ws.cell(row=index+1, column=3, value=record.GetFixedDT())
        ...
        for r_count in range(0, max_rises_count):
            if r_count < len(gl_rises):
                ...
                ws.cell(row=index+1, column=5 + r_count*4 + 1, value=gl_rise[0][0].total_seconds())
                ws.cell(row=index+1, column=5 + r_count*4 + 2, value=gl_rise[0][1])
                ws.cell(row=index+1, column=5 + r_count*4 + 3, value=gl_rise[1][0].total_seconds())
                ws.cell(row=index+1, column=5 + r_count*4 + 4, value=gl_rise[1][1])
            else:
                ws.cell(row=index+1, column=5 + r_count*4 + 1, value="NA")
                ws.cell(row=index+1, column=5 + r_count*4 + 2, value="NA")
                ws.cell(row=index+1, column=5 + r_count*4 + 3, value="NA")
                ws.cell(row=index+1, column=5 + r_count*4 + 4, value="NA")
